...
- When the user asks for generated images or a generated design direction with visual assets, use the `imagegen` skill/tool for bitmap hero/product/ad imagery unless blocked. Do not create or register SVG files for generated visual assets, logos, diagrams, ads, page art, PDF art, or VSL art. Use PNG/WebP/JPG bitmaps from imagegen/imagegen-composite, provided/licensed sources, or real artifact screenshots. Pillow/PIL is not an OfferOS creative generator: do not use it to author customer-facing image assets. It may only inspect, crop, resize, or composite an asset whose creative source was already imagegen/provided/licensed, and provenance must remain tied to that source. HTML/CSS or generated-by-code may support real diagrams/previews only; they cannot satisfy primary conversion visuals such as the product bundle, offer-stack bundle, product mockup, hero/VSL thumbnail, buyer-situation photo, or ad creatives.
...
- Sales page framework: load `references/direct-response-framework.md` before `copy.md` for every complete paid front-end offer. The sales page must follow the canonical spine: message match, hook/VSL, problem, agitation, failed alternatives, mechanism, proof/demo, before/after, product reveal, offer stack, guarantee, objections, final close.
- Sales page: load `assets/templates/sales-page/section-map.md` and start from `assets/templates/sales-page/page-skeleton.html`. Fill every required `data-offeros-section`; preserve the direct-response hero and buy-box offer-stack contracts exactly, keep proof/demo before the main offer stack, then add/remove only optional sub-blocks inside those sections.
