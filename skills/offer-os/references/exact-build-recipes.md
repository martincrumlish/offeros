...
1. Use the plugin-owned OfferOS Studio dispatcher and builders as the production source of truth. Start with `scripts/offeros.py`, then call the relevant studio builder: `build_visual_asset_plan.py`, `build_sales_page_studio.py`, `build_email_sequence.py`, `build_workbook.py`, `build_vsl_deck.js`, `generate_delivery_dashboard.py`, and `validate_offer_outputs.py`.
...
5. Sales Page Studio builds `index.html` from controlled AC-inspired direct-response components and records `quality.salesPage.studio: "sales-page-studio-v2"` and `quality.salesPage.componentLibrary: "ac-inspired-direct-response-v1"`.
...
8. Build `index.html` only by running `scripts/build_sales_page_studio.py` or the compatibility wrapper `scripts/build_sales_page.py`. Do not hand-write `index.html`, do not start from a blank page, and do not replace the locked hero or offer-stack shell with a custom layout.
...
- If `index.html` was not created by Sales Page Studio or lacks `data-offeros-page-kit="v1"`, `data-offeros-builder="offeros-page-kit-builder-v1"`, `data-offeros-sales-page-studio="sales-page-studio-v2"`, and `data-offeros-vsl-placement="main-column-stacked"`, rebuild through the Sales Page Studio builder.
