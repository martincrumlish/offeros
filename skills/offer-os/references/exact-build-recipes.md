...
9. Create `copy-plan.json` before `copy.md`, then run `scripts/build_copy.py`. `copy-plan.json.sectionPlan[].copyBlocks` must contain finished buyer-facing sales copy, not outline notes. Apply the Copy Critic rubric to rendered `copy.md`. Create `visual-asset-plan.json` and `visual-asset-plan.md` v2 only after `copy-plan.json`, clean `copy.md`, `copy-blueprint.md`, and `sales-page-blueprint.json` exist and the copy passes. Reusing sales-page images as the default visual pool fails deep mode.
...
3. Strategy + Copy Studio creates `offer-architecture.json`, `offer-architecture.md`, `copy-plan.json`, clean written `copy.md`, internal `copy-blueprint.md`, and `sales-page-blueprint.json`. Load `references/copywriter-quality-bar.md` before writing `copy-plan.json`, then apply `references/copy-critic-rubric.md` after rendering. `copy-plan.json` must use `framework: "modern-brunson-long-form-v1"`, `standaloneCopyRequired: true`, and `vslDependency: "optional-supporting-asset"`. The Copy Studio source, finished copy, critic pass, and rendered section blueprint must exist before sales-page visual planning.
...
4. For every planned visual, list these exact fields: `artifactTarget`, `filePath`, `visualKind`, `copyAnchor`, `conversionJob`, `aspectRatio`, `aspectRatioReason`, `displayIntent`, `maxDisplayHeight`, `textRule`, `requiredTool`, `requiredAction`, `imagegenRequired`, `fallbackAllowed`, `source/provenance`, `finalPixelsGeneratedBy`, `localPostprocess`, `localCreativeOverlay`, `reusePermission`, `artifactSpecific`, and `generationPrompt`.
...
6. For `## Sales Page Visuals`, every row must include a `copyAnchor` matching a real Copy Studio section/page section from `copy-plan.json`, `copy-blueprint.md`, `sales-page-blueprint.json`, and the page skeleton, such as `hero`, `problem`, `agitation`, `failed-alternatives`, `mechanism`, `proof`, `product`, `offer-stack`, `pricing`, `guarantee`, or `faq`.
7. Use `mixed-direct-response-v1` by default:
   ...
   - every buyer-facing sales-page visual row must explicitly say: `requiredTool: imagegen`, `requiredAction: call-imagegen-skill-tool`, `imagegenRequired: true`, and `fallbackAllowed: false`
   - the `generationPrompt` for each buyer-facing sales-page visual must start with: `CALL THE imagegen SKILL/TOOL FOR THIS EXACT ROW. Save the imagegen output to [filePath].`
   - set `source/provenance: imagegen-final`, `finalPixelsGeneratedBy: imagegen`, `localCreativeOverlay: false`, and `localPostprocess` limited to crop, resize, compression, or format-conversion for every buyer-facing sales-page image row, including `hero-vsl-frame`, `buyer-situation-photo`, `structured-panel`, `comparison-visual`, `mechanism-diagram`, `proof-demo-visual`, `product-mockup`, `offer-stack-bundle`, and `ad-creative` rows unless the asset is user-provided or licensed
   ...
   - do not set `productionMethod`, `source/provenance`, or fallback notes for any buyer-facing sales-page image row to `Pillow`, `PIL`, `html-css`, `canvas`, `screenshot`, `generated-by-code`, or `manual`; HTML/CSS can be used as live page layout, and screenshots can be QA evidence or real artifact previews, but they must not be saved as sales-page support images or counted as generated sales-page visuals. Pillow/PIL is never allowed as an output authoring method.
...
9. If agents are authorized, load `references/agent-dispatch.md` and dispatch imagegen visual workers after this plan exists. Use separate workers for page visuals, PDF visuals, VSL visuals, and ad visuals. Each worker gets `offer-architecture.md`, `design.md`, `copy-plan.json`, clean `copy.md`, `copy-blueprint.md`, `sales-page-blueprint.json`, `visual-asset-plan.md`, the frozen final `assets/logo.png`, the product blueprint/page archetypes, VSL slide plan, ad angle map, and its assigned output folder. Do not give workers any other logo image. The main agent keeps ownership of integration, manifest registration, and QA.
...
- If `copy-plan.json`, clean written `copy.md`, `copy-blueprint.md`, or `sales-page-blueprint.json` does not exist, stop before creating `visual-asset-plan.md`.
...
- If any buyer-facing sales-page visual lacks `requiredTool: imagegen`, `requiredAction: call-imagegen-skill-tool`, `imagegenRequired: true`, or `fallbackAllowed: false`, rebuild the plan.
- If any buyer-facing sales-page visual prompt does not directly instruct `CALL THE imagegen SKILL/TOOL FOR THIS EXACT ROW`, rewrite the row before image production.
...
- If any buyer-facing sales-page support visual, product bundle, offer-stack bundle, product mockup, hero/VSL thumbnail, buyer-situation photo, mechanism diagram, failed-alternatives comparison, proof/demo visual, structured panel, or ad creative is planned as PIL/HTML/CSS/canvas/screenshot/generated-by-code/manual, stop and replace it with an `imagegen-final` job or a provided/licensed asset.
- If `index.html` uses a local image in `data-offeros-page-visual`, `data-offeros-video-thumbnail`, or `data-offeros-product-bundle`, that source must be registered in `offer-os.json` with `provenance: imagegen-final` or provided/licensed provenance, `finalPixelsGeneratedBy: imagegen`, `localCreativeOverlay: false`, and only crop/resize/compression/format-conversion local post-processing. Otherwise rebuild the image through imagegen before marking the page complete.
...
2. Load `references/copy-studio-framework.md`, `references/copy-plan-contract.md`, `references/copywriter-quality-bar.md`, `references/copy-critic-rubric.md`, `references/product-reveal-framework.md`, `references/feature-benefit-rules.md`, `references/objection-matrix.md`, and `references/direct-response-framework.md` before writing copy.
...
4. Write finished buyer-facing sales copy into every `sectionPlan[].copyBlocks` row. Do not write notes like "this section explains", "buyer can see", "conversion job", "belief shift", or instructions for a later copywriter. The plan must contain 1,800+ buyer-facing copy-block words before rendering.
5. Run `scripts/build_copy.py` to render exact page-copy `copy.md`, internal `copy-blueprint.md`, and `sales-page-blueprint.json`. Do not hand-author `copy.md` as the source of truth in deep mode.
   - `copy.md` must use bracketed section blocks such as `[hero]...[/hero]`, `[new-insight]...[/new-insight]`, `[mechanism]...[/mechanism]`, and `[offer-stack]...[/offer-stack]`.
   - `sales-page-blueprint.json` must set `copySourcePath: "copy.md"`, `copySectionContract: "exact-copy-sections-v1"`, and `pageRendersExactCopy: true`.
   - The sales page builder must render from typed `copy-plan.json.sectionPlan[].copyBlocks` first, using `copy.md` as the readable delivery artifact and exact-text check. Do not infer the whole layout from Markdown bullets.
   - The sales page builder must render the text inside those blocks exactly. It may add layout, images, icons, and CTAs from typed `cta` blocks or `[cta]...[/cta]`, but it must not summarize, rewrite, delete, compress, slice to three cards, or invent replacement copy.
6. `copy-plan.json` must include a specific epiphany/new insight before the named unique mechanism, proof/demo before price and offer stack, structured product reveal, feature-benefit-reason rows, how-it-works steps, value-explained offer-stack items, 7+ real objections, and a non-fake urgency basis.
7. `copy-blueprint.md` must include `# Section Blueprint` as a Markdown table with one row for every required page section. Each row must include `sectionId`, `conversionJob`, `targetWords`, `beliefShift`, `proofOrObjection`, `visualKind`, `copyAnchor`, and `ctaRole`. This blueprint is rendered from `copy-plan.json`; it prevents a generic wall of text and becomes the source for `visual-asset-plan.md` v2.
8. `copy-blueprint.md` must include these exact headings:
   - `# Sales Page Type`
   - `# Section Blueprint`
   - `# Hero`
   - `# VSL Setup`
   ...
   - `# Final CTA`
9. `copy.md` must be clean written long-form sales copy only, with bracketed page-copy section markers. It must not contain `# Section Blueprint`, schema notes, or section tables; those belong in `copy-blueprint.md`. It must contain 2,500+ customer-facing words; the target range is 3,500-5,500.
10. Apply `references/copy-critic-rubric.md`. If the copy is thin, repetitive, generic, VSL-dependent, missing feature-benefit-reason detail, missing 7+ real objections, or contains meta copy, revise `copy-plan.json` and rerun `scripts/build_copy.py`.
11. `sales-page-blueprint.json` must be generated from Copy Studio source and compatible with `schemas/sales-page-blueprint.schema.json`. It must select exactly one Page Kit archetype from `classic-vsl-longform`, `modern-vsl-software`, `one-page-tripwire`, `challenge-workshop`, or `toolkit-workbook`; set the same value in `pageKitArchetype`; set `checkout.target` or `checkoutTarget` to `#checkout`; set `orderForm` to false; and map required plus optional sections to approved Page Kit blocks.
12. Write `theme.json` from `schemas/theme.schema.json` or a compatible Page Kit theme preset in `assets/page-kit/themes/`. It must select exactly one theme preset from `light-saas-direct-response`, `classic-direct-response`, `bold-webinar`, `premium-editorial`, `fitness-performance`, or `creator-workshop`. It controls style, typography, spacing, card treatment, and motion; it must not alter the conversion structure.
13. Build `index.html` only by running `scripts/build_sales_page.py`. Do not hand-write `index.html`, do not start from a blank page, and do not replace the locked hero or offer-stack shell with a custom layout.
14. Use the exact stacked VSL-first hero v2 contract from `assets/templates/sales-page/section-map.md`: `data-offeros-hero-layout="stacked-vsl"`, `data-offeros-hero-contract="stacked-vsl-hero-v2"`, `data-offeros-template="offeros-stacked-vsl-v2"`, `oo-hero oo-hero-stacked-vsl`, centered buyer filter, prehead, H1, benefit lead, `data-offeros-hero-copy-stack`, large centered `.oo-vsl-frame` with `data-offeros-hero-video`, `data-offeros-hero-video-prominence="primary"`, `data-offeros-hero-video-size="large"`, thumbnail marked `data-offeros-video-thumbnail`, play button marked `data-offeros-video-play`, caption marked `data-offeros-video-caption`, `data-offeros-price-strip` below the video, CTA to `#checkout`, and `data-offeros-trust-row`.
15. Use the exact offer-stack buy-box contract from `assets/templates/sales-page/section-map.md`: `id="checkout"` or `data-offeros-buy-section`, product bundle visual, `data-offeros-offer-checklist` with 8+ deliverables, `data-offeros-value-row`, large `data-offeros-stack-cta`, and `data-offeros-access-copy`. Do not include an embedded checkout, order form, payment fields, or credit-card form.
16. Keep every required `data-offeros-section` marker from `assets/templates/sales-page/section-map.md`, including the separate `agitation`, `failed-alternatives`, `mechanism`, and pre-offer `proof` sections.
   Proof/demo must appear before the main offer stack.
17. Follow this DOM order: `hero`, `vsl`, `problem`, `agitation`, `failed-alternatives`, `mechanism`, `proof`, `before-after`, `product`, `offer-stack`, `fit`, `pricing`, `guarantee`, `faq`, `final-cta`.
18. Write unique benefit copy for every offer-stack item. Do not map a repeated sentence over multiple cards.
19. Use direct-response composition rules:
    ...
    - Buyer-facing image provenance: every image rendered into `data-offeros-page-visual`, `data-offeros-video-thumbnail`, or `data-offeros-product-bundle` must be registered in `offer-os.json` as imagegen-final/provided/licensed. Local Pillow/canvas/HTML/CSS/screenshots/code-generated PNGs are blocked by `scripts/build_sales_page.py`; if the visual is wrong, regenerate or edit it with imagegen instead of repairing it locally.
    ...
    - VSL section: 80-220 words, 3-5 bullets, and one CTA. Do not put the whole sales letter in the VSL section. If the hero already contains the main VSL frame, do not label the later section "Watch this first"; use a label like "What the walkthrough covers" or "The breakdown in plain English". Do not call the VSL a pitch in buyer-facing copy.
...
20. Hero order is fixed: concise buyer filter, concise prehead, H1, subheadline/lead, large 16:9 VSL frame with caption, then price strip/CTA/trust elements below the video. Do not put purchase CTAs or random buy bullets above the video.
21. Use typed section components instead of one repeated card/list row everywhere: problem symptoms, agitation impact list, failed-alternatives contrast, mechanism steps, proof cards, feature-benefit grid, how-it-works timeline, differentiated offer-stack component, guarantee pills, FAQ accordions, and final CTA.
    The offer-stack component must read `copy-plan.json.offerStack.items`, not only the visible bullets in the offer-stack copy block. Render the first 3 items as featured package pieces, render the remaining deliverables as compact checklist rows, show item value tags when provided, and then show the normally/today value row. Do not render the whole offer stack as 10+ identical generic boxes.
22. Include at least 7 FAQ objections, at least 4 CTA placements, at least 3 post-hero CTA placements, and at least 2,500 visible words for `direct-response-long-form-vsl`.
23. Mark every FAQ item with `data-offeros-faq-item`.
24. Mark every CTA link or button with `data-offeros-cta`; mark post-hero CTA placements with `data-offeros-post-hero-cta`; purchase CTAs must link to `#checkout` by default.
25. Set `quality.copy.studio: "copy-studio-v1"`, `quality.copy.framework: "modern-brunson-long-form-v1"`, `quality.copy.standaloneCopyRequired: true`, `quality.copy.vslDependency: "optional-supporting-asset"`, `quality.copy.finishedCopySource: "copy-plan.sectionPlan.copyBlocks"`, `quality.copy.copyBlocksBuyerFacing: true`, `quality.copy.copyCriticRubric: "copy-critic-rubric-v1"`, `quality.copy.copyCriticPassed: true`, `quality.copy.minimumCopyWords: 2500`, `quality.copy.targetCopyWords: "3500-5500"`, `quality.copy.hasNewInsight: true`, `quality.copy.hasUniqueMechanism: true`, `quality.copy.hasFailedAlternatives: true`, `quality.copy.hasProofBeforeOffer: true`, `quality.copy.hasFeatureBenefitBreakdown: true`, and `quality.copy.hasObjectionMatrix: true`.
26. Set `quality.salesPage.visibleWordCount`, `objectionCount`, `ctaCount`, `postHeroCtaCount`, `offerStackItemsUnique`, `sectionDepthChecked`, `repeatedTextChecked`, `copyBlueprintPresent: true`, `copyStudioUsed: true`, `copyPlanPath: "copy-plan.json"`, `copySourcePath: "copy.md"`, `copySectionContract: "exact-copy-sections-v1"`, `pageRendersExactCopy: true`, `standaloneCopyRequired: true`, `framework: "modern-brunson-long-form-v1"`, `pageFramework: "direct-response-long-form-v1"`, `compositionContract: "direct-response-composition-v2"`, `heroContract: "stacked-vsl-hero-v2"`, `heroLayout: "stacked-vsl"`, `heroTemplate: "offeros-stacked-vsl-v2"`, `heroVideoFrame: "large-16x9"`, `heroVideoProminenceChecked: true`, `offerStackContract: "direct-response-buy-box-v1"`, `pageKit: "offeros-page-kit-v1"`, `pageKitBuilder: "offeros-page-kit-builder-v1"`, `pageKitArchetype`, `themePreset`, `pageKitBlueprintUsed: true`, `themeTokensUsed: true`, `navigationPolicy: "no-section-nav"`, `iconSystem: "lucide-icons-v1"`, `iconLibrary: "lucide"`, `imageDisplay: "viewport-constrained-v1"`, `buyerFacingImagePolicy: "imagegen-final-or-provided-v1"`, `pageVisualImagegenFinalRequired: true`, `localCreativeImageFallbackAllowed: false`, `eyebrowPolicy: "sparse-key-signposts-v1"`, `eyebrowAlignment: "centered-with-section-heading"`, `vslSectionCommand: "overview-not-watch-first"`, `checkoutTarget: "#checkout"`, `vslPlacement: "main-column-stacked"`, and `orderFormIncluded: false`.

Stop conditions:

- If `copy.md` contains `# Section Blueprint`, schema notes, section tables, or any internal blueprint fields, revise before page or visual planning. The blueprint belongs in `copy-blueprint.md`.
...
- If `copy-plan.json.sectionPlan[].copyBlocks` are notes, internal labels, repeated filler, or under 1,800 buyer-facing words total, revise before rendering.
...
- If `copy.md` is under 2,500 words, VSL-dependent, generic, repetitive, or missing major section depth, revise `copy-plan.json` and rerun `scripts/build_copy.py`.
