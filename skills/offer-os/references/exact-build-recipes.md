...
9. Create `copy-plan.json` before `copy.md`, then run `scripts/build_copy.py`. Create `visual-asset-plan.json` and `visual-asset-plan.md` v2 only after `copy-plan.json`, clean `copy.md`, `copy-blueprint.md`, and `sales-page-blueprint.json` exist. Reusing sales-page images as the default visual pool fails deep mode.
...
4. Run `scripts/build_copy.py` to render clean written `copy.md`, internal `copy-blueprint.md`, and `sales-page-blueprint.json`. Do not hand-author `copy.md` as the source of truth in deep mode.
...
6. `copy-blueprint.md` must include `# Section Blueprint` as a Markdown table with one row for every required page section. Each row must include `sectionId`, `conversionJob`, `targetWords`, `beliefShift`, `proofOrObjection`, `visualKind`, `copyAnchor`, and `ctaRole`. This blueprint is rendered from `copy-plan.json`; it prevents a generic wall of text and becomes the source for `visual-asset-plan.md` v2.
7. `copy-blueprint.md` must include these exact headings:
   - `# Sales Page Type`
   - `# Section Blueprint`
   - `# Hero`
   - `# VSL Setup`
   ...
   - `# Final CTA`
8. `copy.md` must be clean written long-form sales copy only. It must not contain `# Section Blueprint`, schema notes, or section tables; those belong in `copy-blueprint.md`.
9. `sales-page-blueprint.json` must be generated from Copy Studio source and compatible with `schemas/sales-page-blueprint.schema.json`. It must select exactly one Page Kit archetype from `classic-vsl-longform`, `modern-vsl-software`, `one-page-tripwire`, `challenge-workshop`, or `toolkit-workbook`; set the same value in `pageKitArchetype`; set `checkout.target` or `checkoutTarget` to `#checkout`; set `orderForm` to false; and map required plus optional sections to approved Page Kit blocks.
10. Write `theme.json` from `schemas/theme.schema.json` or a compatible Page Kit theme preset in `assets/page-kit/themes/`. It must select exactly one theme preset from `light-saas-direct-response`, `classic-direct-response`, `bold-webinar`, `premium-editorial`, `fitness-performance`, or `creator-workshop`. It controls style, typography, spacing, card treatment, and motion; it must not alter the conversion structure.
11. Build `index.html` only by running `scripts/build_sales_page.py`. Do not hand-write `index.html`, do not start from a blank page, and do not replace the locked hero or offer-stack shell with a custom layout.
