...
These values are not interchangeable. Deep mode requires real `imagegen-final` bitmap assets for buyer-facing sales-page images when the user asks for generated design direction, generated images, ad images, or supporting images. Every image rendered into `data-offeros-page-visual`, `data-offeros-video-thumbnail`, or `data-offeros-product-bundle` must be `imagegen-final`, `provided`, or `licensed` unless `imagegen-composite` was performed natively by imagegen and records `imagegenNativeComposite: true`, `finalPixelsGeneratedBy: imagegen`, and `localCreativeOverlay: false`. Product bundles, offer-stack bundles, product mockups, hero/VSL thumbnails, buyer-situation photos, mechanism diagrams, failed-alternatives comparisons, proof/demo visuals, structured panels, and ad creatives must not be PIL/HTML/CSS/canvas/screenshot/generated-by-code/manual PNGs. `html-css`, `generated-by-code`, `manual`, and `screenshot` can support live page layout, worksheets, rendered previews, QA evidence, or real artifact screenshots, but they do not count as AI image generation and cannot satisfy buyer-facing sales-page image slots. Pillow/PIL-authored image artifacts are not allowed in deep OfferOS runs; Pillow may only inspect, crop, resize, compress, or format-convert already-sourced assets. SVG files are not allowed generated visual assets or fallbacks.
...
- a sales-page support visual, product bundle, offer-stack bundle, product mockup, hero/VSL thumbnail, buyer-situation photo, mechanism diagram, failed-alternatives comparison, proof/demo visual, structured panel, or ad creative is made as a PIL/HTML/CSS/canvas/screenshot/generated-by-code/manual PNG instead of `imagegen-final`/provided/licensed
- `index.html` uses a local image in `data-offeros-page-visual`, `data-offeros-video-thumbnail`, or `data-offeros-product-bundle` that is not registered in `offer-os.json` with imagegen-final/provided/licensed metadata
...
Fails if it is visually thin, nonresponsive, missing core sections, reads like a short product page, skips the internet-marketing/direct-response arc, asks for the sale before mechanism/proof, uses a nice branded hero instead of the required stacked VSL-first direct-response hero, uses a two-column/split SaaS hero with a small side video, turns the VSL area into a wall of text, labels a post-hero VSL section "Watch this first" after the hero already contains the primary VSL, adds sticky/hover section navigation or header nav links, uses an eyebrow/pill on every section, leaves visible section eyebrows/pills floating left of centered H2s, replaces the buy-box checklist stack with generic cards/pricing panels, uses plain box grids without Lucide icon markers/checkmarks, leaves sales-page images unconstrained at full source size, places contained images inside mismatched colored mattes, repeats generic card blurbs, has invisible/low-contrast text, or contains scaffold remnants.
